...
public class RenderTest extends Module {
    public final BooleanSetting box3D = new BooleanSetting("3D", "Draws an 3D box around the entity.", false);
    public final BooleanSetting outline2D = new BooleanSetting("2D", "Draws an 2D outline around the entity.", false);
    ...
    public RenderTest() {
        ...
        this.addSettings(box3D, outline2D, line);
    }
    ...
    public void onRender(RenderEvent.Pre event) {
        ...
        if (box3D.isEnabled()) {
            RenderUtil.draw3DBox(event.getMatrix4f(), RenderUtil.smoothen(mc.player, mc.player.getBoundingBox()), new Color(255, 0, 0, 125));
        } else if (outline2D.isEnabled()) {
            RenderUtil.draw2DOutline(event.getMatrix4f(), mc.player, new Color(255, 0, 0, 255));
            ...
            for (Entity entity : mc.world.getEntities()) {
                if (entity instanceof AnimalEntity) {
                    ...
                    RenderUtil.draw3DLineFromPlayer(event.getMatrix4f(), start, end, new Color(255, 0, 0, 255));
                }
            }
        }
    }
